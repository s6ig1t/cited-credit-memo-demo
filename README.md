...
```
Aloan.Demo.Core        Plain domain models. No logic, no dependencies.
Aloan.Demo.Analysis     Deterministic ratio calculators + policy thresholds.
                        No AI, no HTTP — fully unit-testable in isolation.
Aloan.Demo.Agents       The only project that talks to Claude.
                        ExtractionAgent: PDF in, structured + cited data out.
                        MemoAgent: computed ratios in, cited narrative out.
Aloan.Demo.Api          ASP.NET Core Web API. Orchestrates the three stages
                        behind one endpoint. Serves the HTML memo viewer.
Aloan.Demo.Tests        36 unit tests against Analysis — boundary values,
...
1. Clone the repo and open `AloanDemo.sln`.
2. Set your API key via .NET User Secrets on the `Aloan.Demo.Api` project:
   ```
   dotnet user-secrets set "AnthropicApiKey" "sk-ant-..." --project Aloan.Demo.Api
   ```
   (Or right-click the project in Visual Studio → *Manage User Secrets*.)
3. Run `Aloan.Demo.Api` (F5, or `dotnet run --project Aloan.Demo.Api`).
